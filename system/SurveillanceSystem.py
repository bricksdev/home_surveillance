...
parser.add_argument('--imgDim', type=int, help="Default image dimension.", default=120)
...
class SurveillanceSystem(object):
    ...
    def process_frame(self, camera):
        ...
        while not stop:
            ...
            if frame == None or np.array_equal(frame,
                                               camera.tempFrame):  # Checks to see if the new frame is the same as the previous frame
                continue
    ...
    def detected_recoginer_face(self, frame_data):
        """This function performs all the frame proccessing.
                It reads frames captured by the IPCamera instance,
                resizes them, and performs 1 of 5 functions"""
        logger.debug('Processing detecting Frame')

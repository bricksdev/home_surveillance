    ...
    def detect_movement(self, frame, get_rects):
        ...
        if self.history <= 3:  # Let the camera warm up
            ...
            if get_rects == True:  # Return peoplerects without frame
                ...
                return occupied, frame
        elif self.history > 4000 and len(
                self.peopleRects) == 0:  # Recalculate background model every 4000 frames only if there are no people in frame
            self.previousFrame = self.currentFrame
            self.currentFrame = gray
            self.history = 0
        ...
        for c in cnts:
            ...
            if (h) > (w):
                occupied = True
                if (h) > (
                    1.5 * w):  # Most likely a person, this can be made strictor (average human ratio 5.9/1.6 = h/w = 3.6875)
                    self.person = True
